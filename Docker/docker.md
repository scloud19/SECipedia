Docker
(Cheat sheet influenced by https://github.com/wsargent/docker-cheat-sheet)
...
* [`docker build`](https://docs.docker.com/reference/commandline/build) creates image from Dockerfile.
    Z: -t option allows us to create a build with a specific name
...
### Dockerfile (all everything until next bold is in this category)
CMD [“apache2ctl”, “-D”, “FOREGROUND”] <— Running apache in the foreground

Build Cache: When building from a docker file, it will utilize prebuilt images if they are already made.

Reducing the amount of images created in dockerfile.  (This also decreases the size of the build)
By making one RUN command through chaining, we only create one “RUN” image.  Otherwise, we get an image for every command
RUN apt-get update && apt-get install -y /
     apache2 /
     apache2-utils /
     && apt-get clean /

Making sure docker files stay lean
